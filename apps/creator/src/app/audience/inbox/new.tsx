...
export default function NewConversationScreen() {
  ...
  async function handleStartConversation(artist: PublicArtistSummary) {
    ...
    try {
      ...
      router.replace(`/audience/inbox/${conversationId}` as never);
    } catch (error) {
      Alert.alert(
  ...
  return (
    <Screen scroll={false} header={<AppHeader variant="detail" title="New message" fallbackRoute="/audience/inbox" />} contentContainerStyle={styles.screenContent}>
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <InboxRow
                  ...
                  onPress={() =>
                    router.replace(
                      `/audience/inbox/${item.conversationId}` as never,
                    )
                  }
                />
              )}
