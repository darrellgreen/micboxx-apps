...
export async function signInWithDrupal(): Promise<MicboxxSession> {
  if (!hasLiveDrupalConfig()) {
    throw new Error(
      "Missing live Drupal auth configuration. Set EXPO_PUBLIC_DRUPAL_BASE_URL, EXPO_PUBLIC_DRUPAL_OAUTH_CLIENT_ID, and EXPO_PUBLIC_DRUPAL_OAUTH_SCOPE.",
    );
  }

  const expoGo = isExpoGo();
  const appReturnUri = buildAppReturnUri();
  const redirectUri = expoGo
    ? buildRelayRedirectUri()
    : env.drupalOAuthRedirectUri || appReturnUri;
